...
_gosleek_complete() {
    ...
    local flags="-t --target -l --list -T --templates -id --tid --tags --severity -e --exclude -v --verbose -vv --silent -o --output -f --format -c --concurrency -r --rate-limit --timeout -p --proxy --verify-ssl --oob --oob-provider --ceye-key --ceye-domain --allow-external-hosts --resume --log-file --log-level --plugins-only --plugin --redact -s --filter-severity --filter-tags --follow-redirects -H --header --output-dir --wordlist-dir"
