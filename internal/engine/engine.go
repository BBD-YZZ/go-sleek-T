...
type Scanner struct {
	...
	aiResults        sync.Map       // 存储 AI 分析结果：resultID → *ai.AnalyzeResponse
	aiPendingResults sync.Map       // 存储待过滤的原始结果：resultID → *types.Result
...
// GetFilteredResults 根据 AI 置信度阈值过滤结果。
// 在 WaitForAI() 完成后调用，返回通过 AI 确认的结果。
// 如果 aiConfidence <= 0，返回 nil 表示不过滤。
func (s *Scanner) GetFilteredResults() []*types.Result {
	if s.aiConfidence <= 0 {
		// 未设置置信度阈值，返回 nil 表示不过滤
		return nil
	}

	var filtered []*types.Result
	s.aiResults.Range(func(key, value interface{}) bool {
		resultID := key.(string)
		resp := value.(*ai.AnalyzeResponse)
		if resp.Confidence >= s.aiConfidence {
			// 找到原始结果
			if result, ok := s.aiPendingResults.Load(resultID); ok {
				filtered = append(filtered, result.(*types.Result))
			}
		}
		return true
	})
...
func (s *Scanner) executePlugin(ctx context.Context, p plugin.Plugin, target string, ti *placeholder.TargetInfo, eng *placeholder.Engine) *types.Result {
	...
	// [增强] 添加 panic recovery 保护，防止卡死的插件阻塞 worker
	defer func() {
		if r := recover(); r != nil {
			s.debug("插件 panic 恢复: %s, error=%v", p.Meta().ID, r)
		}
	}()
	...
	if s.oobAvailable && s.oobProvider != nil {
		...
		pctx.OOB = plugin.NewOOBHandle(s.oobProvider, s.client)
		pctx.Ceye = pctx.OOB // 兼容旧插件
	}
...
// [修复] 使用 ":::" 分隔符与 runJob 保持一致，确保断点续扫正确跳过已完成任务
