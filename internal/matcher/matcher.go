...
func evalSingle(m types.Matcher, ctx *MatchContext) (bool, string) {
	...
	case "xpath":
		matched, evidence = matchXPath(m, ctx)
	case "css":
		matched, evidence = matchCSS(m, ctx)
	case "html":
		matched, evidence = matchHTML(m, ctx)
	default:
		matched = false
	}
...
// matchXPath matches response body against XPath expressions.
// Reuses applyXPath from extractor.go for consistency.
func matchXPath(m types.Matcher, ctx *MatchContext) (bool, string) {
	if len(m.XPath) == 0 {
		return false, ""
	}
	data := getPart(m, ctx)
	...
	ev := ""
	if len(evidence) > 0 {
		ev = "xpath: " + strings.Join(evidence, ", ")
	}
	return result, ev
}

// matchCSS matches response body against CSS selectors.
// Reuses applyCSSSelector from extractor.go for consistency.
func matchCSS(m types.Matcher, ctx *MatchContext) (bool, string) {
	if len(m.CSS) == 0 {
		return false, ""
	}
	data := getPart(m, ctx)
	...
	ev := ""
	if len(evidence) > 0 {
		ev = "css: " + strings.Join(evidence, ", ")
	}
	return result, ev
}

// matchHTML matches response body against HTML tag patterns.
// Extracts text content from specified tags.
func matchHTML(m types.Matcher, ctx *MatchContext) (bool, string) {
	if len(m.HTML) == 0 {
		return false, ""
	}
	data := getPart(m, ctx)
	...
	for _, tag := range m.HTML {
		...
		if len(matches) > 1 {
			any = true
			evidence = append(evidence, matches[1])
		} else {
			all = false
		}
	}
...
func SubstituterMatcherPlaceholders(matchers []types.Matcher, eng *placeholder.Engine) []types.Matcher {
	...
	for i, m := range matchers {
		...
		out[i].XPath = replaceStrSlice(out[i].XPath, eng)
		out[i].CSS = replaceStrSlice(out[i].CSS, eng)
		out[i].HTML = replaceStrSlice(out[i].HTML, eng)
	}
	return out
}
