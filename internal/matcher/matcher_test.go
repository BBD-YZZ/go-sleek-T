...
// TestXPathMatcher verifies xpath matcher functionality.
func TestXPathMatcher(t *testing.T) {
	htmlBody := `<html><body><div id="token">ABC123</div><span class="secret">Secret456</span></body></html>`
	...
	tests := []struct {
		...
		{
			name: "xpath non-match",
			m: types.Matcher{
				Type:  "xpath",
				XPath: []string{"//div[@id='missing']"},
			},
			want: false,
		},
		{
			name: "xpath with text() extraction",
			m: types.Matcher{
				Type:  "xpath",
				XPath: []string{"//span[@class='secret']/text()"},
			},
			want:     true,
			evidence: "Secret456",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			matched, evidence := Evaluate([]types.Matcher{tt.m}, "or", ctx)
			if matched != tt.want {
				t.Errorf("xpath matcher matched=%v, want %v, evidence=%s", matched, tt.want, evidence)
			}
			if tt.want && evidence == "" {
				t.Error("expected non-empty evidence for positive match")
			}
		})
	}
}

// TestCSSMatcher verifies css matcher functionality.
func TestCSSMatcher(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			matched, evidence := Evaluate([]types.Matcher{tt.m}, "or", ctx)
			if matched != tt.want {
				t.Errorf("css matcher matched=%v, want %v, evidence=%s", matched, tt.want, evidence)
			}
		})
	}
}

// TestHTMLMatcher verifies html matcher functionality.
func TestHTMLMatcher(t *testing.T) {
	htmlBody := `<html><body><title>Test Page</title><div class="content">Hello World</div></body></html>`
	ctx := NewMatchContext(200, htmlBody, "", 0)
	...
	tests := []struct {
		...
		{
			name: "html multiple tags (OR)",
			m: types.Matcher{
				Type:      "html",
				HTML:      []string{"title", "h1"},
				Condition: "or",
			},
			want:     true,
			evidence: "Test Page",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			matched, evidence := Evaluate([]types.Matcher{tt.m}, "or", ctx)
			if matched != tt.want {
				t.Errorf("html matcher matched=%v, want %v, evidence=%s", matched, tt.want, evidence)
			}
		})
	}
}

// TestXPathCSSMatcherCondition verifies and/or conditions work correctly.
func TestXPathCSSMatcherCondition(t *testing.T) {
	htmlBody := `<html><body><div id="a">Alpha</div><span id="b">Beta</span></body></html>`
	ctx := NewMatchContext(200, htmlBody, "", 0)

	// AND: both must match
	mAnd := types.Matcher{
		Type:      "xpath",
		XPath:     []string{"//div[@id='a']", "//span[@id='b']"},
		Condition: "and",
	}
	matched, _ := Evaluate([]types.Matcher{mAnd}, "and", ctx)
	if !matched {
		t.Error("AND xpath: both should match")
	}

	// OR: at least one must match
	mOr := types.Matcher{
		Type:      "xpath",
		XPath:     []string{"//div[@id='a']", "//span[@id='missing']"},
		Condition: "or",
	}
	matched, _ = Evaluate([]types.Matcher{mOr}, "or", ctx)
	if !matched {
		t.Error("OR xpath: at least one should match")
	}

	// AND with one failing
	mAndFail := types.Matcher{
		Type:      "xpath",
		XPath:     []string{"//div[@id='a']", "//span[@id='missing']"},
		Condition: "and",
	}
	matched, _ = Evaluate([]types.Matcher{mAndFail}, "and", ctx)
	if matched {
		t.Error("AND xpath: should fail when one doesn't match")
	}
}
