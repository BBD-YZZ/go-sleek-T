...
type GlobalConfig struct {
	...
	Proxy          string `yaml:"proxy"` // HTTP proxy URL
...
// The cookie is set dynamically after Probe() succeeds.
type DnslogConfig struct {
	Cookie string `yaml:"cookie"` // PHPSESSID cookie (set by engine after probe)
}
