...
// BatchAnalyzeResult 表示批量分析中单个条目的结果（含错误信息）。
type BatchAnalyzeResult struct {
	// RequestIndex 对应 BatchRequest 中的索引。
	RequestIndex int `json:"index"`
	// Response 分析结果，失败时为 nil。
	Response *AnalyzeResponse `json:"response,omitempty"`
	// Error 分析失败时的错误信息。
	Error string `json:"error,omitempty"`
}

// BatchRequest 是批量分析的输入，包含多条待分析结果。
type BatchRequest struct {
	// Results 待分析的漏洞结果列表。
	Results []*AnalyzeRequest `json:"results"`
}

// BatchResponse 是批量分析的输出，包含每个条目的分析结果。
type BatchResponse struct {
	// Results 按索引顺序对应的分析结果列表，失败条目为 nil。
	Results []BatchAnalyzeResult `json:"results"`
}
...
type Provider interface {
	...
	// AnalyzeBatch 批量分析多条漏洞结果，减少 API 调用次数。
	// 当 provider 不支持批量时，可返回 nil, nil 让调用方退回到逐条分析。
	AnalyzeBatch(ctx context.Context, req *BatchRequest) (*BatchResponse, error)
