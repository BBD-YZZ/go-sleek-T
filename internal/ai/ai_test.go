...
func TestOpenAIProvider_AnalyzeBatch_Success(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		if r.Method != http.MethodPost {
			t.Errorf("期望 POST 请求，实际 %s", r.Method)
		}
			...
			Choices: []openaiChatChoice{
				{
					Message: openaiChatMessage{
						Role:    "assistant",
						Content: `[{"index":0,"confident":true,"confidence":0.9,"evidence":"证据A","risk_assessment":"高风险"},{"index":1,"confident":false,"confidence":0.3,"evidence":"","risk_assessment":"低置信度"}]`,
					},
				},
			},
		}
	...
	batchReq := &BatchRequest{
		Results: []*AnalyzeRequest{
			{TemplateID: "b1", Target: "http://example.com", RawRequest: "GET / HTTP/1.1\r\n\r\n", RawResponse: "HTTP/1.1 200 OK\r\n\r\n"},
			{TemplateID: "b2", Target: "http://example.org", RawRequest: "GET / HTTP/1.1\r\n\r\n", RawResponse: "HTTP/1.1 200 OK\r\n\r\n"},
		},
	}

	batchResp, err := p.AnalyzeBatch(ctx, batchReq)
	if err != nil {
		t.Fatalf("AnalyzeBatch 不应报错: %v", err)
	}
	if len(batchResp.Results) != 2 {
		t.Fatalf("期望 2 条结果，实际 %d", len(batchResp.Results))
	}
	if !batchResp.Results[0].Response.Confident {
		t.Error("期望第 0 条 confident=true")
	}
	if batchResp.Results[0].Response.Confidence != 0.9 {
		t.Errorf("期望置信度 0.9，实际 %f", batchResp.Results[0].Response.Confidence)
	}
	if batchResp.Results[1].Response.Confidence != 0.3 {
		t.Errorf("期望置信度 0.3，实际 %f", batchResp.Results[1].Response.Confidence)
	}
}

func TestOpenAIProvider_AnalyzeBatch_SingleResult(t *testing.T) {
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		// 单条应退回到 Analyze 路径
		resp := openaiChatResponse{
			Choices: []openaiChatChoice{
				{
					Message: openaiChatMessage{
						Role:    "assistant",
						Content: `{"confident":true,"confidence":0.95,"evidence":"test","risk_assessment":"test"}`,
					},
				},
			},
		}
	...
	batchReq := &BatchRequest{
		Results: []*AnalyzeRequest{
			{TemplateID: "s1", Target: "http://example.com"},
		},
	}
	...
	if len(batchResp.Results) != 1 {
		t.Fatalf("期望 1 条结果，实际 %d", len(batchResp.Results))
	}
	if !batchResp.Results[0].Response.Confident {
		t.Error("期望 confident=true")
	}
...
func TestOpenAIProvider_AnalyzeBatch_NilRequest(t *testing.T) {
	p := NewOpenAIProvider("test-key", "https://api.openai.com/v1", "gpt-4o", "openai", 10)
	_, err := p.AnalyzeBatch(context.Background(), nil)
	if err == nil {
		t.Fatal("期望传入 nil 时返回错误")
	}
}

func TestBuildBatchAnalysisPrompt_NonEmpty(t *testing.T) {
	reqs := []*AnalyzeRequest{
		{TemplateID: "t1", Target: "http://example.com", RawRequest: "GET / HTTP/1.1\r\n\r\n", RawResponse: "HTTP/1.1 200 OK\r\n\r\n"},
		{TemplateID: "t2", Target: "http://example.org", RawRequest: "GET / HTTP/1.1\r\n\r\n", RawResponse: "HTTP/1.1 200 OK\r\n\r\n"},
	}
	prompt := BuildBatchAnalysisPrompt(reqs)
	if prompt == "" {
		t.Fatal("批量提示词不应为空")
	}
	if !contains(prompt, "2 条结果") {
		t.Error("提示词应包含结果数量")
	}
	if !contains(prompt, "t1") {
		t.Error("提示词应包含 template ID")
	}
	if !contains(prompt, "t2") {
		t.Error("提示词应包含第二个 template ID")
	}
}

func TestParseBatchResponse_JSONArray(t *testing.T) {
	originals := []*AnalyzeRequest{
		{TemplateID: "a"},
		{TemplateID: "b"},
		{TemplateID: "c"},
	}
	content := `[{"index":0,"confident":true,"confidence":0.9,"evidence":"e1"},{"index":2,"confident":false,"confidence":0.2}]`
	batchResp, err := parseBatchResponse(content, originals)
	if err != nil {
		t.Fatalf("parseBatchResponse 不应报错: %v", err)
	}
	if len(batchResp.Results) != 3 {
		t.Fatalf("期望 3 条结果，实际 %d", len(batchResp.Results))
	}
	if batchResp.Results[0].Response == nil {
		t.Error("期望第 0 条有结果")
	}
	if batchResp.Results[0].Response.Confidence != 0.9 {
		t.Errorf("期望第 0 条置信度 0.9，实际 %f", batchResp.Results[0].Response.Confidence)
	}
	// 第 1 条没有对应响应，应为 nil
	if batchResp.Results[1].Response != nil {
		t.Error("期望第 1 条为 nil")
	}
	if batchResp.Results[2].Response == nil {
		t.Error("期望第 2 条有结果")
	}
}
...
func (m *mockProvider) AnalyzeBatch(ctx context.Context, req *BatchRequest) (*BatchResponse, error) {
	if req == nil || len(req.Results) == 0 {
		return nil, fmt.Errorf("批量分析请求不能为空")
	}
	results := make([]BatchAnalyzeResult, len(req.Results))
	for i := range results {
		results[i] = BatchAnalyzeResult{
			RequestIndex: i,
