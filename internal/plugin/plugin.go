...
type Context struct {
	...
	OOB            OOBHandle                // 无回显验证（ceye/dnslog/callbackred API 轮询）
	// Ceye 是 OOB 字段的别名，为兼容旧插件保留（一次性 deprecation）
	Ceye           OOBHandle                // DEPRECATED: 请使用 OOB 字段
	Log            Logger                   // 日志接口
	...
	LogLevel       int                      // 日志级别（0=info, 1=verbose, 2=debug）
	Proxy          string                   // 代理 URL
...
type Reporter interface {
	...
	LogHTTPRequest(stepName string, stepIndex int, reqIndex int, req *http.Request)
	// LogHTTPResponse 记录 HTTP 响应（便捷方法，自动转换 *http.Response 为 raw 格式）
	LogHTTPResponse(stepName string, stepIndex int, reqIndex int, resp *http.Response, body string, elapsed time.Duration)
}
...
type OOBHandle interface {
	...
	URL() string
	// Token 提供商凭据（ceye token / dnslog PHPSESSID / callbackred key）
	Token() string
	// Provider 提供商名称（ceye/dnslog/callbackred）
	Provider() string
