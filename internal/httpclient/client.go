...
type Client struct {
	...
	insecure       bool // whether TLS verification is disabled
}
...
func (c *Client) doRequest(ctx context.Context, baseURL string, rawReq *RawRequest) (*Response, error) {
	...
	if strings.HasPrefix(pathLower, "http://") || strings.HasPrefix(pathLower, "https://") {
		// Case 1: full URL in path — still respect allowExternal SSRF check
		parsed, err := url.Parse(rawReq.Path)
		if err != nil {
			return nil, fmt.Errorf("SSRF防护: 无效的目标 URL %s，已拒绝", rawReq.Path)
		}
		// Strip query/fragment for host comparison
		targetURL, _ := url.Parse(baseURL)
		if !isSameTarget(parsed.Host, targetURL.Host) {
			// External host — only allow if explicitly enabled
			if !c.allowExternal {
				return nil, fmt.Errorf("SSRF防护: 绝对URL指向外部主机 %s，已拒绝（使用 --allow-external-hosts 启用）", parsed.Host)
			}
		}
		fullURL = rawReq.Path
	} else {
