...
func Validate(tmpl *types.Template) []ValidationError {
	...
	// OOB validation — support all configured providers
	if tmpl.OOB != nil {
		validProviders := map[string]bool{
			"ceye": true, "dnslog": true, "callbackred": true,
		}
		if !validProviders[strings.ToLower(tmpl.OOB.Provider)] {
			add("error", "oob.provider", fmt.Sprintf("unsupported provider %q, allowed: ceye/dnslog/callbackred", tmpl.OOB.Provider))
		}
	}
