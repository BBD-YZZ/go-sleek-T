...
// LoadFileWithVerify loads a template and optionally verifies its SHA256 signature.
// If expectedSHA is non-empty, the template must match; otherwise it loads regardless.
func LoadFileWithVerify(path string, expectedSHA string) (*types.Template, error) {
	data, err := os.ReadFile(path)
	if err != nil {
		return nil, err
	}
	...
	if expectedSHA != "" && !VerifySign(data, expectedSHA) {
		return nil, fmt.Errorf("template signature mismatch: %s (expected %s, got %s)",
			path, expectedSHA, tmpl.SHA256)
	}
...
// LoadDirWithVerify loads all YAML templates from a directory with optional signature verification.
// expectedSHAs is a map of file basename to expected SHA256.
// If verification is enabled and a template's signature doesn't match, it's skipped with a warning.
func LoadDirWithVerify(dir string, expectedSHAs map[string]string) ([]*types.Template, error) {
	var templates []*types.Template
	var walkErr error

	verified := false
	for _, sha := range expectedSHAs {
		if sha != "" {
			verified = true
			break
		}
	...
	err := filepath.Walk(dir, func(path string, info os.FileInfo, err error) error {
		...
		var tmpl *types.Template
		if verified {
			t, loadErr := LoadFileWithVerify(path, expectedSHAs[info.Name()])
			if loadErr != nil {
				logutil.Log("warn", "跳过", "签名验证失败 %s: %v", path, loadErr)
				return nil
			}
			tmpl = t
		} else {
			var loadErr error
			tmpl, loadErr = LoadFile(path)
			if loadErr != nil {
				logutil.Log("warn", "跳过", "无法加载的模板 %s: %v", path, loadErr)
				return nil
			}
		}
