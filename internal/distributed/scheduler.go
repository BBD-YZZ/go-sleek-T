...
type Scheduler struct {
	...
	wg        sync.WaitGroup
	ctx       context.Context // 传播给所有子操作，Stop 时取消
	...
	// 聚合所有 JobResult 中的 types.Result（全局去重+汇总）
	aggregatedMu sync.Mutex
	aggregated   []*types.Result
}
...
func NewScheduler(logger LoggerIface) *Scheduler {
	...
	return &Scheduler{
		...
		resultCh:  make(chan *JobResult, 512),
		logger:    logger,
...
func (s *Scheduler) DispatchJob(job *ScanJob) <-chan *JobResult {
	...
	go func() {
		...
		results := pt.results
		pt.mu.Unlock()
		// 即使没有结果也发送一个 error 结果，确保 channel 不阻塞
		if len(results) == 0 {
			results = []*JobResult{{JobID: job.JobID, Error: "no results from workers"}}
		}
	...
	// 启动结果收集协程（只收集不写 channel）
	s.wg.Add(1)
	go s.collectResults(job.JobID, pt)

	return ch
}

// collectResults 收集单个任务的所有 Worker 响应。
// 仅将结果存入 pt.results 并聚合到全局，不写 resultCh（避免 race）。
// 完成后关闭 pt.done，由 DispatchJob 的 goroutine 负责推送结果。
func (s *Scheduler) collectResults(jobID string, pt *pendingTask) {
	defer s.wg.Done()
...
func (s *Scheduler) dispatchToWorkers(job *ScanJob, cfg *config.GlobalConfig,
	...
	// 使用 sync.Once 防止多次 close done channel
	var closeDone sync.Once

	// 使用 WaitGroup 追踪 Worker 完成情况
	var wg sync.WaitGroup
	wg.Add(len(workers))

	for i, worker := range workers {
		go func(idx int, w *NodeInfo, tgs []string) {
			defer wg.Done()
			s.executeOnWorker(job, w, tgs, cfg)
			// Worker 完成后，通知 collectResults 可以关闭 done
			closeDone.Do(func() {
				if pt, ok := s.pending.Load(job.JobID); ok {
					if p, ok := pt.(*pendingTask); ok {
						close(p.done)
...
func (s *Scheduler) executeOnWorker(job *ScanJob, worker *NodeInfo, targets []string, cfg *config.GlobalConfig) {
	...
	if result == nil {
		// 远程和本地都失败，发送错误结果
		result = &JobResult{JobID: job.JobID, Error: "all execution paths failed"}
	}
...
// executeLocallyOnTargets 在本地执行扫描（无 Worker 时的回退），从磁盘加载模板和插件。
func (s *Scheduler) executeLocallyOnTargets(cfg *config.GlobalConfig, jobID string, targets []string) *JobResult {
	start := time.Now()
	templates, err := LoadTemplates(cfg.TemplateDir, nil)
	if err != nil {
		return &JobResult{JobID: jobID, Error: "load templates: " + err.Error()}
	}
	plugins := plugin.All()
	scanner := engine.NewScanner(cfg, 0, engine.OOBConfig{}, "", false)
	results := scanner.Run(s.ctx, templates, plugins, targets)
	duration := time.Since(start).Milliseconds()
	stats := JobStats{
...
func (s *Scheduler) executeLocally(job *ScanJob, cfg *config.GlobalConfig,
	...
	start := time.Now()
	scanner := engine.NewScanner(cfg, 0, engine.OOBConfig{}, "", false)
	results := scanner.Run(s.ctx, templates, plugins, job.Targets)
	duration := time.Since(start).Milliseconds()
...
// aggregateResult 将单个 JobResult 中的漏洞结果聚合到全局列表。
// 使用 Go map 去重（按 target+templateID 作为 key），避免跨 Worker 重复。
func (s *Scheduler) aggregateResult(result *JobResult) {
	s.aggregatedMu.Lock()
	defer s.aggregatedMu.Unlock()

	// 去重
	seen := make(map[string]bool, len(s.aggregated)+len(result.Results))
	for _, r := range s.aggregated {
		seen[r.Target+":::"+r.TemplateID] = true
	}
	for _, r := range result.Results {
		key := r.Target + ":::" + r.TemplateID
		if !seen[key] {
			seen[key] = true
			s.aggregated = append(s.aggregated, r)
		}
	}
}
...
// GetResults 返回所有收集到的聚合结果（跨 Worker 去重）。
func (s *Scheduler) GetResults() []*types.Result {
	s.aggregatedMu.Lock()
	defer s.aggregatedMu.Unlock()
	cp := make([]*types.Result, len(s.aggregated))
	copy(cp, s.aggregated)
	return cp
}

// GetJobResults 返回所有 JobResult（原始分片结果）。
func (s *Scheduler) GetJobResults() []*JobResult {
	s.resultMu.Lock()
...
// GetAggregatedCount 返回聚合结果数量。
func (s *Scheduler) GetAggregatedCount() int {
	s.aggregatedMu.Lock()
	defer s.aggregatedMu.Unlock()
