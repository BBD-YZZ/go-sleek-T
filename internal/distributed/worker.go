...
func (w *Worker) HandleJob(job *ScanJob) *JobResult {
	...
	results := scanner.Run(funcCtx(w.ctx), templates, plugins, targets)
...
// funcCtx returns ctx if non-nil, otherwise context.Background().
// Used to gracefully handle workers that haven't been Start()ed (e.g. in tests).
func funcCtx(ctx context.Context) context.Context {
	if ctx == nil {
