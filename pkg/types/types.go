...
type Matcher struct {
	...
	// xpath/css/html only: xpath expressions or css selectors
	XPath []string `yaml:"xpath,omitempty"`
	CSS   []string `yaml:"css,omitempty"`
	HTML  []string `yaml:"html,omitempty"` // tag names for html matcher
}

// Extractor pulls data from responses for reuse.
type Extractor struct {
	Name   string   `yaml:"name"`
	Type   string   `yaml:"type"` // regex/word/kval/json/cookie/xpath/css/html
	...
	// Scope controls variable visibility: "step" (local to request block),
	// "global" (visible across all steps/templates), default "local".
